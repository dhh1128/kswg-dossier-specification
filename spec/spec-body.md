...
### Threshold mechanics
A joint issuance MUST satisfy an m-ary threshold operator defined in the edge section of the dossier ACDC. A schema MAY define the required threshold and the set of possible signers. Alternatively, a schema MAY defer these definitions to the dossier instance, allowing the threshold rules to be set only when the issuance is proposed.
...
* `M`: a [[ref: threshold operator]] declaring that issuance is accomplished by satisfying an endorser count. The count MUST be expressed via a corresponding field on the edge, `m`. The presence of this operator requires that the set of corresponding signers MUST be represented in the edges of the edge group to which the operator is attached.

    Valid *potential* signers (as opposed to *actual* signers in the edge) MAY be enumerated in advance. When they are, the enumeration MUST appear in the attributes (root `a` object) section of the ACDC and MUST consist of an array of AIDs. If a field named `mgrp` appears as a property on the same edge as `M`, it MUST name the attributes field where this enumeration occurs. Otherwise, the enumeration MUST be given in a field named `mgrp` in the attributes section.

    With an enumerated set of potential signers, the `M` operator expresses an *m of n* approval pattern: `m` is the threshold, and the cardinality of `mgrp` is the logical upper bound *n*. For example, a judicial decision could be jointly issued by *m* of *n* justices, where the AIDs of the judges are enumerated and *m* is a majority. An `M` operator without an enumerated set of potential signers MAY instead be combined with the `Q` operator (see below) to model an unbounded number of potential signers who nonetheless must be qualified in some way.
* `RM`: a [[ref: revocation operator]] declaring that revocation is accomplished by satisfying a revoker count. `RM` has parallel semantics to `M`: its corresponding numeric field on the edge is `rm`, and its potential revokers are enumerated in an `rmgrp` field in the attributes section (or in an attribute field whose name is given by the `rmgrp` field on the edge). This flexibility lets the set of revokers be identical to the endorser set used for `M`, overlap it, or be entirely disjoint, and lets the threshold for revocation differ from the threshold for issuance.
* `Q`: a qualification operator that defines a standard of proof that signers MUST meet. When this operator is present, the edge MUST also contain a `qschema` property describing the required proof, plus a `qev` array enumerating edges of evidence presented by each signer as proof of qualification.
* `FIN`: a [[ref: finalization operator]] that signals whether a verifier should expect a finalization event in a KEL. Recording a finalization event in the KEL lets verifiers predict where aggregate evidence may be collected for easy review. Without it, a verifier MUST collect evidence of joint issuance signatures from disparate locations.
...
If a finalization event is present, a verifier SHOULD use it as the definitive proof of issuance. If it is absent, a verifier MUST poll the individual KELs of all possible participants to determine whether the threshold has been met.
...
This structure means that any child object within an ACDC can be replaced by its SAID without altering the SAID of the parent object and without invalidating the digital signature on the container. The holder or issuer of a dossier can therefore generate redacted versions of the ACDC that selectively hide sensitive information while remaining cryptographically verifiable. In joint issuance, redaction does not affect the validity of member seals anchored in KELs, because those seals point to the immutable SAID of the root dossier.
